...
ChromeContentBrowserClient::ChromeContentBrowserClient(
  ...
  extra_parts_.push_back(new VivaldiContentBrowserClientParts);
...
content::BrowserMainParts* ChromeContentBrowserClient::CreateBrowserMainParts(
  ...
  main_parts->AddParts(VivaldiBrowserMainExtraParts::Create());
...
std::string ChromeContentBrowserClient::GetStoragePartitionIdForSite(
  ...
  if (site.SchemeIs(content::kGuestScheme) || site.SchemeIs("chrome-extension"))
...
bool ChromeContentBrowserClient::CanCreateWindow(
  ...
  *no_javascript_access = false;
...
#if BUILDFLAG(ENABLE_EXTENSIONS)
  // NOTE(andre@vivaldi.com) : This is needed to run the content setting update
  // below.
  if (!vivaldi::IsVivaldiRunning() &&
  ...
  // handling popups.
  if (!vivaldi::IsVivaldiRunning() &&
      MaybeBlockPopup(web_contents, opener_top_level_frame_url, &nav_params,
