...
base::string16 GetAppName(const base::string16& install_path);
...
void GetShellIntegrationEntries(
  ...
  entries->push_back(std::make_unique<RegistryEntry>(
      start_menu_entry, vivaldi::GetAppName(chrome_exe.value())));
  ...
  entries->push_back(std::make_unique<RegistryEntry>(
      ...
      vivaldi::GetAppName(chrome_exe.value())));
...
void ShellUtil::AddDefaultShortcutProperties(const base::FilePath& target_exe,
  ...
  if (!properties->has_arguments()) {
    const base::string16& arguments = vivaldi::install_static::GetArguments();
    if (!arguments.empty())
