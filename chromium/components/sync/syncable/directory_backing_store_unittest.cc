...
void MigrationTest::SetUpVersion91Database(sql::Database* connection) {
  ...
  ASSERT_TRUE(connection->Execute(
    ...
    "INSERT INTO 'share_info' VALUES('nick@chromium.org','nick@chromium.org',"
       ...
       "93368420856x',NULL);"
    // Vivaldi
    "ALTER TABLE share_version ADD COLUMN vivaldi_version int;"
    "UPDATE share_version SET vivaldi_version = 1;"
    "ALTER TABLE metas ADD COLUMN ""unique_notes_tag VARCHAR;"
    // End Vivaldi
      ));
  // clang-format on
  ASSERT_TRUE(connection->CommitTransaction());
