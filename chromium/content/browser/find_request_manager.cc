...
FrameTreeNode* GetParent(FrameTreeNode* node) {
  if (node->parent()) {
    if (auto* contents = WebContentsImpl::FromFrameTreeNode(node->parent())) {
      if (contents->GetContentsMimeType() == kPDFMimeType) {
        // NOTE(andre@vivaldi.com) : MimeHandlerGuestViews has two nodes. One
        // below holding the PDF, skip this and proceed. This was for Vivaldi
        // bug VB-49066.
