...
void RenderWidgetHostViewMac::SmartMagnify(
  ...
  if (vivaldi::IsVivaldiRunning()) {
    // NOTE(espen@vivaldi.com): Route smart magify events the same way as other
    // guesture events so that they will zoom the conter view and not the root
    // (UI) view. See SendGesturePinchEvent() and OnGestureEvent().
    if (ShouldRouteEvents()) {
...
void RenderWidgetHostViewMac::Undo() {
  if (vivaldi::IsVivaldiRunning()) {
    // Use the delegate as is done done for copy/cut/paste/selectAll
    // Needed from ch67. I suspect chromium will change to the same.
...
// Added by Vivaldi (ch69)
void RenderWidgetHostViewMac::Delete() {
  if (auto* delegate = GetFocusedRenderWidgetHostDelegate()) {
    delegate->Delete();
  }
}
