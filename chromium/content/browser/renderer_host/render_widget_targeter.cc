...
void RenderWidgetTargeter::FindTargetAndDispatch(
  ...
  if (request_in_flight_) {
    if (vivaldi::IsVivaldiRunning()) {
      ...
      // any.
      result = delegate_->FindTargetSynchronously(root_view, event);
      if (result.view == root_view) {
        // The assert can be too strict especially with
        // --use-viz-hit-test-surface-layer. Remove it if so.
        DCHECK(!result.should_verify_result);
        if (!result.should_query_view && !result.should_verify_result)
          goto process_result;
      }
    }
    if (!requests_.empty()) {
